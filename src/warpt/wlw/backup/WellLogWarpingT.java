    ...
    for (int il=0,ip=0; il<nl; ++il) {
      for (int jl=il+1; jl<nl; ++jl,++ip) {
        ...
        for (int ik=1; ik<n; ++ik) {
          if (is[ik]-is[ik-1]==0 || js[ik]-js[ik-1]==0) {
            ++m;
          } else {
            ki[ik-m] = is[ik];
            kj[ik-m] = js[ik];
...
/*
        ...
        float[] kj = new float[n];
        int klo, khi;
        for (int ik=1; ik<n; ++ik) {
          if (is[ik]-is[ik-1]==0) {
            klo = khi = ik;
            while (klo>0 && is[klo]-is[klo-1]==0) --klo; 
            while (khi<n-1 && is[khi]-is[khi-1]==0) ++khi; 
            ki[ik] = (float)((khi-ik)*(is[klo]) + (ik-klo)*(is[khi]))/(float)(khi-klo);
            kj[ik] = (float)js[ik];
          } else if (js[ik]-js[ik-1]==0) {
            klo = khi = ik;
            while (klo>0 && js[klo]-js[klo-1]==0) --klo; 
            while (khi<n-1 && js[khi]-js[khi-1]==0) ++khi; 
            kj[ik] = (float)((khi-ik)*(js[klo]) + (ik-klo)*(js[khi]))/(float)(khi-klo);
            ki[ik] = (float)is[ik];
          } else {
            ki[ik] = (float)is[ik];
            kj[ik] = (float)js[ik];
...
*/
        ...
        */
        ...
        // Find total minimum accumulated error 
        float te = FLT_MAX;
        for (int l=lmin,ll=0; l<=lmax; ++l,++ll) {
          if ((kmax+l)%2==0) {
            if (d[kmax][ll]<te) {
...
/*
    ...
    PixelsView pv = sp.addPixels(new Sampling(nk,.001,0.0),new Sampling(nl,1.0,1.0),r);
    ...
    ramp(0,1,d);
    PointsView pv = pp.addPoints(d,r[1]);
    pv.setLineColor(Color.RED);
    pv = pp.addPoints(d,r[0]);
...
*/
    ...
    A a = new A(ps);
    Ml m = new Ml(sigma,nk,nl,ps,nls);
    ...
    */
    ...
    //copy(s,zt);
    //invertShiftsT2Z(cl,zt); // gives z(t)
    
    /*
    PlotPanel pp = new PlotPanel();
    float[] d = new float[nk];
    ramp(0,1,d);
    PointsView pv = pp.addPoints(d,s[1]);
    pv.setLineColor(Color.RED);
    ...
    */
    ...
    //m.test(s);
    //m.subtractMeanOverLogs(s);
    //System.out.println("s after clean");
    //dump(s);

/*
    sp = new SimplePlot(SimplePlot.Origin.UPPER_LEFT);
    sp.setSize(700,380);
    sp.addColorBar("cleaned");
    pv = sp.addPixels(new Sampling(nk,.001,0.0),new Sampling(nl,1.0,1.0),cl);
    pv.setColorModel(ColorMap.JET);
    pv.setInterpolation(PixelsView.Interpolation.NEAREST);
*/
   ...
   */
  public float[][] applyShiftsZ(float[][] f, float[][] s) {
    int nk = f[0].length;
    int nl = f.length;
    ...
    for (int il=0; il<nl; ++il) {
      ...
      for (int ik=0; ik<nk; ++ik) {
        ...
        // Time (in samples) at which to interpolate log.
        float tk = ik+s[il][ik];

        // Nearest-neighbor interpolation.
        int jk = (int)(tk+0.5f);
        if (0<=jk && jk<nk && f[il][ik]!=_vnull) {
          g[il][jk] = f[il][ik];
        }
        /* 
  ...
  private static class Ml implements CgSolver.A {
    Ml(double sigma, int nk, int nl, Pairs[] ps, boolean[][] nls) {
    ...
    /*
    // inverse interpolates to get r, smol in r, ii to get s
    public void subtractMeanOverLogsR(float[][] x) {
      int nk = x[0].length;
      int nl = x.length;
      float[] c = new float[nk];
      float[][] r = new float[nl][nk];
      float[][] cl = new float[nl][nk];
      copy(x,r);
      zero(_s);
      invertShifts(cl,r);
    ...
    public void subtractMeanOverLogsR(float[][] x) {
      ...
      for (int il=0; il<nl; ++il) {
        for (int ik=0; ik<nk; ++ik) {
          ...
          float t = ik-x[il][ik];
          int it = (int)t;
          if (0<=it && it<nk && c[it]>0 && !_nls[il][ik]) {
            if (it<nk-1 && c[it+1]>0) {
              float dt = t-it;
              x[il][ik] -= (1.0f-dt)*_s[it]/c[it]+dt*_s[it+1]/c[it+1];
            } else {
              x[il][ik] -= _s[it]/c[it];
            }
          }
  ...
  /*
  private static void buildA(int nl, int nk, Pairs[] ps) {
    int np = ps.length; // number of log pairs
    int nij = 0;
    for (int ip=0; ip<np; ++ip) { // for all log pairs, ...
      Pairs p = ps[ip];
  ...
  */
  private static void applyLhsAtA(Pairs[] ps, float[][] x, float[][] y) {
    int nk = x[0].length; // number of depths
    int nl = x.length; // number of logs
    int np = ps.length; // number of log pairs
    zero(y); // zero y before accumulating below
    double[][] ata = new double[nl*nk][nl*nk];
    int nij = 0;
    for (int ip=0; ip<np; ++ip) { // for all log pairs, ...
      Pairs p = ps[ip];
      float[] ws = p.ws;
    ...
    DMatrix aD = new DMatrix(a);
    DMatrix atD = aD.transpose();
    DMatrix ataD = atD.times(aD);

    //DMatrixEvd ataE = ataD.evd();
    //DMatrixEvd ataE = new DMatrixEvd(ataD);
/*
    DMatrix dD = ataE.getD();
    DMatrix vD = ataE.getV();
    double[][] v = vD.get();
    double[][] d = dD.get();
    SimplePlot sp = new SimplePlot(SimplePlot.Origin.UPPER_LEFT);
    sp.setSize(700,380);
    sp.addColorBar("D");
    PixelsView pv = sp.addPixels(v);
    pv.setColorModel(ColorMap.JET);
    pv.setInterpolation(PixelsView.Interpolation.NEAREST);
*/
  ...
  private static void cleanShifts(float[] s, float[] cl) {
    int n1 = s.length;
    //System.out.println("cleaning shifts");
  ...
  private static void cleanShiftsT2Z(float[] s) {
    int n1 = s.length;
    for (int i1=1; i1<n1; ++i1) {
      if (s[i1]>s[i1-1]+0.999f)
        s[i1] = s[i1-1]+0.999f;
  ...
  private static void invertShifts(
    InverseInterpolator ii, float[] u, float[] t, float[] cl, float[] s) 
  {
    cleanShifts(s,cl);
    int n1 = s.length;
    for (int i1=0; i1<n1; ++i1) 
  ...
  private static void invertShiftsT2Z(
    InverseInterpolator ii, float[] u, float[] t, float[] cl, float[] s) 
  {
    cleanShifts(s,cl);
    int n1 = s.length;
  ...
  public static void invertShiftsT2Z(float[][] cl, float[][] s) {
    int n1 = s[0].length;
    int n2 = s.length;
    ...
    for (int i2=0; i2<n2; ++i2)
      invertShifts(ii,u,t,cl[i2],s[i2]);
  }
  public static void invertShifts(float[][] cl, float[][] s) {
    int n1 = s[0].length;
    int n2 = s.length;
    float[] u = rampfloat(0.0f,1.0f,n1);
    float[] t = zerofloat(n1);
  ...
  public static void invertShiftsR2S(float[][] s) {
    int n1 = s[0].length;
    int n2 = s.length;
    float[] u = rampfloat(0.0f,1.0f,n1);
    float[] t = zerofloat(n1);
